...
from source.parquet_core import execute_tail_strategy
...
class TestParquetTailFunc(unittest.TestCase):
    ...
    def test_when_args_equals_to_None_is_expected_to_return_one_element(self):
        ...
        filtered_element = execute_tail_strategy(dataframe, None)
    ...
    def test_when_args_equals_to_five_is_expected_to_return_zero_elements(self):
        ...
        filtered_element = execute_tail_strategy(dataframe, 5)

        self.assertEqual(5, len(filtered_element.index))

    def test_when_args_is_bigger_then_dataframe_len_is_expected_to_return_all_elements(self):
        dataframe = create_dataframe(5)
        filtered_element = execute_tail_strategy(dataframe, 10)

        self.assertEqual(5, len(filtered_element.index))

    def test_when_args_equals_one_is_expected_to_return_the_last_elements(self):
        dataframe = create_dataframe(5)
        filtered_element = execute_tail_strategy(dataframe, 1)

        self.assertEqual(1, len(filtered_element.index))
        self.assertEqual(4, filtered_element.index)
